...
function showBadCustomers()
{
    var ajaxRequest = new XMLHttpRequest();
    ajaxRequest.open('GET', "ShowBadUsers", true);
    ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    ajaxRequest.onreadystatechange = function ()
    {
        if (ajaxRequest.readyState === 4 && (ajaxRequest.status === 200))
        {
            console.log("Successful showBadCustomers() GET request on DB.");
        ...
        else if (ajaxRequest.status === 400)
        {
            document.getElementById("main_div").innerHTML = "<br><div id=\"inner_main_div\">\n"
                                + "<br><br><br><br><center><p> Something went wrong with the request, send an email requesting to solve the error 400 problem\n\
                                   on Show Bad Customers</p></center>\n"
        ...
        else
        {
            document.getElementById("main_div").innerHTML = "<br><div id=\"inner_main_div\">\n"
                            ...
                            + "</div>\n";
            console.log("showBadCustomers() POST request for username check on DB failed.");
        }
    };
    ajaxRequest.send();
}